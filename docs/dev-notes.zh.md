...
## Backlog（2026-07-07 刷新）

- **P0：Stacks 展开/收起动画**——根因已诊断：Collapsed 成员坐标未停放到堆位（首次展开从旧位置飞来、展开缺"从堆里出来"的动画）。修法：收起先 MoveIcon 到堆格再 Collapsed；展开自堆位飞出；收起动画完成回调后再隐藏。
- Stacks v2：分组依据加日期/大小、堆 hover 刮擦预览、展开态新文件加入的平滑性。
- 发布 v0.9.0：仓库切公开 + tag + CI 产物验证（检查更新即自动生效）。
- 远期：动态壁纸透传（真透明/合成层大手术）、拖拽图像掠过高 DPI 屏偏小（shell 限制）、分辨率变化原地平滑版、设置窗口深色主题适配。
