...
## Backlog（2026-07-07 下午 V 刷新）

- Stacks v2 剩余：拖文件进堆（手动归类）、多屏叠放策略复核（待机主双屏实用反馈，现状=每窗口对自己图标聚堆；机主拖 Jellyfin 去 TV 就是在试这个）。
- **壁纸透传：spike 全过，实施设计已定稿（"三明治"架构，下个会话动手）**：
  - **输入层** = 现有 WPF MainWindow 原样保留（child of DefView），外部给它设
    WS_EX_LAYERED + SetLayeredWindowAttributes(LWA_ALPHA, 1)——常量 alpha 层窗口
    **整个矩形收输入**（与逐像素无关），鼠标/键盘/IME/OLE 拖放/框选/菜单一行不改；
    近隐形（1/255）肉眼不可见。WPF 自己的上屏渲染就算因 layered+child 废掉也无所谓
    ——反正看不见，RenderTargetBitmap 离屏渲染视觉树与上屏无关。
  - **呈现层** = 新建纯 Win32 子窗口（UlwSpike 同款），WS_EX_LAYERED | WS_EX_TRANSPARENT
    （输入穿透，输入全归上面的 WPF 窗），z 序贴在 WPF 窗之下；帧 = RootGrid 的
    RenderTargetBitmap → premultiplied BGRA DIB → UpdateLayeredWindow。空白区 alpha=0
    真透明（点击归属在输入层，不需要 spike 里 alpha=1 的招）。
  - **帧调度**：静止零推送；动画/交互期间挂 CompositionTarget.Rendering 按脏推帧，
    上限 ~60fps，动画结束推最后一帧后停。
  - **底下透出什么**：Wallpaper Engine 的 WorkerW 在 DefView 后面 → 透出动态壁纸 ✓；
    纯静态壁纸时 ListView 藏着（既有约束：壁纸画在 ListView 背景）→ 底下是黑的 →
    静态场景仍用现有壁纸镜像画进自己帧里。故做成设置项"真透明（动态壁纸透传）"，
    默认关；开了才停镜像+推 alpha 帧。
  - 风险点留验：RenderTargetBitmap 每帧的 CPU 成本（1080p ~8MB/帧拷贝，60fps 动画期
    需实测；可按脏矩形优化）；LWA_ALPHA(1) 的 WPF child 是否影响 WPF 命中测试（理论
    不影响——命中测试走视觉树坐标，不看合成）。
