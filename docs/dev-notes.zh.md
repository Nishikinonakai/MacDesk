...
## 2026-07-16 凌晨：透明直通 spike 成功——动态模式图标层回归 GPU 渲染（机主"图标帧率不如静态壁纸"反馈）

机主亲手 A/B（拨动态壁纸开关对比）后点名：修完节流后图标动画帧率仍不如静态模式，
但 WE 壁纸本身和系统动画都流畅。**架构性差距**：收编模式图标层 = CPU 逐帧光栅化
（RTB+ULW），静态模式 = WPF GPU 直渲——CPU 路径再怎么调也追不上 GPU。

### Spike：WPF 子窗表面清除色透明 → DWM 直接透出 WorkerW 里的 WE
依据 = P0-A"打洞"bug 的反证：非分层 WS_CHILD 的 WPF 窗，Opacity 中间层合成把表面
alpha 打出洞时，DWM 把身后壁纸透了出来——**证明 DefView 子窗的表面 alpha 参与 DWM
合成**。当年"透传证伪"（2026-07-07）只证死了 WS_EX_LAYERED（GDI layered 与 WPF D3D
重定向互斥），没试过"表面清除色 alpha=0"这条。
实现（settings.json `"DynamicTransparent": true`，spike 开关不进 UI）：
- 不收编 WE、不建 presenter：WE 留在 WorkerW 原生位置（DefView 天然透传其下 WorkerW）
- `HwndSource.CompositionTarget.BackgroundColor = Colors.Transparent`（核心一行）
  + Window/RootGrid Background=Transparent + 撤镜像 + 藏原生图标
- 硬件渲染路径全部自然复用镜像模式逻辑（_presenter==null：帧泵不跑、Effect 免费保留、
  BitmapCache 照挂）；`ApplyDesktopBackground` 守卫加 `|| _transparentOn`（否则 8s 轮询
  把镜像铺回来盖住透明——真机踩过）
- 退出（WE 关闭）：清除色恢复 Colors.Black + 回镜像；ExitDynamic 头部挂 ExitTransparentDynamic 幂等

### 真机验证（home-win 1080p60 + WE Raindrops/Web 型）
- **透出实锤**：间隔 3s 两帧全屏 diff = 19% 像素在变（动态壁纸实时播放）；图标/标签/
  堆叠角标/悬停 tooltip 全部正常渲染，无黑底
- **交互即时**：PostMessage 点击图标 → 250ms 内高亮上屏（GPU 直渲无泵无节流）
- **模式切换链**：taskkill WE → 8s 轮询 → transparent OFF + 回镜像 ✓；WE 重启 →
  transparent ON 重进 ✓
- 红线：layout.json SHA256 不变
- **图标动画帧率 = DWM 合成 = 与静态模式/系统动画同级**——机主的诉求从架构上解决

### 待办（转正前的回归清单，机主拍板后做）
双屏/混合 DPI、老 Intel 软渲机器（RenderMode=software 的表面 alpha 行为）、点击转发
交互壁纸（WE 全局钩子在 WindowFromPoint=我们窗口时的行为）、渐隐动画视觉复查（打洞
在透明模式=透出壁纸本身，理论正确）、Win+D/菜单/拖拽/框选全套、分辨率交接、
DynamicNoShadows/NoAnimations 两开关在透明模式下语义失效（阴影免费了）→ 设置页文案重做、
收编路径保留为回退（透明失败自动降级？加表面 alpha 探测）。
**转正后可删**：UlwPresenter、帧泵/脏区跟踪全套、收编 z 序维护——动态模式复杂度大瘦身。

### 排查插曲
- 透明模式首次部署后"没进动态"= `DynamicWallpaper` 主开关是 False——**机主 A/B 对比
  手感时拨的**（这正是机主能精确说出"不如静态壁纸"的原因），诊断日志一条定位。

## 2026-07-16 凌晨 II：透明直通转正 + v1.5.0 发版（机主拍板"动画真的完美流畅了"）

机主亲手体验 spike 后拍板转正+发版。转正内容：
- `DynamicTransparent` 默认 **true**（透明直通=动态壁纸默认路径）；收编模式整套保留为
  逃生舱（settings.json 手工置 false，不进 UI）——万一某环境 DWM 不尊重子窗表面 alpha
 （症状=图标下黑底）一个开关救回。
- **点击转发适配**（"原有功能都别丢"的关键项）：转发目标从 `_weWindow`（收编句柄）改为
  `_weWindow ?? _transparentWe`；透明模式进入时记录 WE 渲染窗、WE 换壁纸重建窗口时
  8s 轮询携新句柄进 EnterTransparentDynamic 刷新。真机验证：Raindrops 天气按钮
  PostMessage 原地点击 → 天气切换（太阳按钮高亮、雨停、全屏 diff 85.8%）。
- `AnimationsSuppressed` 判定扩到透明模式（`_presenter != null || _transparentOn`）——
  语义从"帧率保底"变纯偏好（透明模式 GPU 满速）。
- 设置外观页：**删"禁用图标阴影"行**（透明模式阴影 GPU 免费恒开；字段保留供收编逃生）；
  主开关/禁动画文案重写（透明直通描述 + 偏好定位）。
- 版本 1.4.0 → **1.5.0**。

### 发版前回归（home-win，全绿）
1080p60+4K30 双分辨率：透明进入/透出（两帧 diff 全屏在动）、点击 250ms 高亮、交互壁纸
点击转发、WE kill→镜像/重启→重进、**RenderMode=software 透明同样可用**（老 Intel 自动
软渲机器无需回退——软渲表面 alpha 同样被 DWM 尊重，真机实测 diff 4.2% 动+零黑屏）、
分辨率交接往返 1080p↔4K（每次交接透明自动重进、WE 干净实例原生跟随 resize、四象限
全动=满屏渲染）、设置窗新 UI、layout.json SHA256 全程不变。

### 已知边界（不阻塞，记录备查）
- 被多轮收编 Adopt/Release+taskkill 折腾过的**旧 WE 实例**在分辨率切换后出现过"CEF 视口
  卡 1080p 贴左上"（窗口本身已 4K）——WE 重启即愈，干净实例往返正常；真实用户从收编版
  升级不会撞（升级即重启）。若有用户报"壁纸只占左上角"→ 让他重启 WE。
- WE 按分辨率记壁纸配置：4K 配置的 EVA 视频与 1080p 的 Raindrops 往返后 WE 自行切换，
  是 WE 自家行为。

## 2026-07-16 深夜：issue#9 收尾——缓解四件套合入 main、探针退役、关单（机主拍板"按建议全部处理"）

### 决策依据（5 天半观察期数据，当晚逐项核实）
- 07-11 05:36 清毒重启后机器**未再重启**，期间 **45 次 MacDesk 进程启动**（部署/交接/看门狗，
  每次孵化新 menu host——正是假说中"host 生灭 churn"触发相邻事件）零复发；
- hangwatch **34 次对常驻 host 的管道文件夹构建全绿**（0.3–0.9s，回帧 12–20 万字节）；
  **85 次真实右键构建无一超 8s**（被动 tail 证实在岗：macdesk.log 上次轮转在 07-11 05:35
  早于探针上哨，句柄未漂移）；
- 全网 ~150 次 release 下载**零同类反馈**，issue#9 无人 +1；
- 结论：根因（机器级 shell 坏状态 + 一次性探针免疫之谜）**停追**——不可人工复现、单机、
  A/B 回滚已洗清自家代码，继续追是开放式研究；缓解**合入**——保护环境未知的真实用户
  （重 shell 扩展生态里坏状态先决条件普遍存在），且四件套中"忙/死识别防连杀""探针超时
  不判终身降级"独立于坏状态也是正确修复。

### 合并
- `fix/menu-cold-start`（40d2fbb+749f536，只动 Services/MenuHost.cs +110/-10）--no-ff 合入
  main，零冲突（main 自 308dd63 未碰该文件），Mac 交叉编译 0 warning 0 error。
- 四件套 07-11 当晚对真活卡死实战验证过降级/处决两条路径；**不单独发版，随下个功能版顺车**
  （与 issue#1 当时的处置同款）。正常菜单路径回归在下次真机部署窗口顺带冒烟。

### hangwatch 批判性复盘（为什么退役而不是加强）
- 还它公道：主动模式打的是**受害路径**（常驻 host 管道构建，非当晚免疫的一次性牺牲探针），
  被动模式盯真实右键——5 天全绿是真证据不是安慰剂。
- 但作为长期监控有四个结构性缺陷：①无心跳，静默="没事"/"瞎了"不可分；②macdesk.log 超
  2MB 轮转（rename→.1）会让启动时一次性打开的句柄漂移到旧文件、被动模式**静默失明**
  （帧日志时代 4 天写了 33MB，v1.5.0 后才安静，没踩中纯属运气）；③无告警链路，抓到只写
  本地文件没人看；④7 天自灭+重启不复活（无计划任务）。
- 边际价值≈0：再抓一次只是重复 07-11 已有采样（2 份 minidump+WCT+模块序），真正缺的
  对照实验（host vs 一次性探针**出生时刻** diff——免疫之谜）它不做；且主动探针每 3.5h
  暖机一次（源码自注 side effect），观察者效应可能反而压低自然复发率。
- **合并后产品自带 fallback/处决日志 = 装机量×几十的被动监测网**，坏状态在任何用户机器
  发生都会留下第一现场（"menu v2 … fallback"/"stuck building"），比单机探针强。
- 处置：hangwatch（pid 2708）让它 07-20 11:22 自然到期，不续班；证物（两份带毒 host
  minidump / bisect 日志 / hangwatch 全程日志）保留在 home-win `C:\work\coldtest\`；
...
### 真·重启验证（2026-07-25 12:00，机主授权重启；这台机 ARSO 自动登录，远程可全程核对）

| 时刻（相对开机 12:00:45） | 事件 |
|---|---|
| +13.5s | **登录触发器开火**，MacDesk 进程起（task LastRun=12:00:59，LastResult 运行中）——比 explorer.exe（+13.8s）**还早 0.3s**，正是加了 `requireDefView` 硬化的那个赛跑 |
| +25.5s | 第一行托管代码（`startup args`）——**冷盘 .NET/WPF 起进程用掉 ~12s**，开机 I/O 争抢下的固有成本 |
| +31.3s | 图标层挂上 DefView |
| +36.5s | 28 个图标排完上屏 |

**结果**：`HideIcons=1` 被全新 Explorer 尊重，`SysListView32` 全程 `visible=False`——
**开机没有任何一帧画过原生图标**；图标层 ex=0x00000080（Win+Tab 实测干净，只剩真窗口）；
layout.json 哈希不变。对比老路径：Run 键自启还要先排 Windows 启动项队列（机主实测 40s+）
**才轮到进程启动**，现在是登录后 13.5s 就起。

**剩余可优化（未做，留 backlog）**：冷盘那 12s 是自包含 .NET 76MB 镜像在开机争抢下换页的
代价，R2R 帮不上（自包含发布的框架程序集本来就是 R2R，只有我们自己的 533KB 没预编）。
真要压可以试：把 menu host 的孵化推迟到首屏挂载之后（现在是 OnStartup 就 spawn，冷启动期
它同时在预热 shell 扩展抢 I/O）——收益 1–3s 量级但动的是最脆的子系统（见 issue#9），
没有可靠 A/B 手段前不动。
