...
## Backlog（2026-07-07 下午 III 刷新）

- 下次部署顺带生效：MenuHost 交接期管道冲突修复（代码已在 main，见"下午 III"节）。
- Stacks v2 剩余：拖文件进堆（手动归类）、多屏叠放策略复核（待机主双屏实用反馈，现状=每窗口对自己图标聚堆）。
- 发布 v0.9.0：仓库切公开 + tag + CI 产物验证（检查更新即自动生效）。
- **壁纸透传（下一个硬骨头）候选路线已定**：WS_EX_LAYERED 子窗口 Win8+ 支持
  UpdateLayeredWindow 逐像素 alpha —— WPF 视觉树离屏渲染（RenderTargetBitmap）按帧推
  ULW；空白区 alpha=1（近隐形但保住点击归属，框选不破）；动态壁纸（Wallpaper Engine）
  在我们层下面原生播放。先做最小验证窗（真机证明 ULW 子窗口挂 DefView 能渲染+收输入）
  再动主窗口重构。
