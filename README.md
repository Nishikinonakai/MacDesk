...
- **Use Stacks** (background menu) — macOS-style auto-grouping, with a **Group
  ...
  three real member icons (actual thumbnails, not a flat placeholder); the
  "Other" catch-all (Kind grouping only — Date/Size groups always have real
  members to show) falls back to a generic layered-card glyph with a chevron
  badge, matching Finder. Hover-scrub a pile to cycle its front icon through
  members without opening it. Clicking a pile flies its members out with a
  350 ms animation; clicking again gathers them back before collapsing.
  Doesn't touch the canonical layout — turning Stacks off restores your exact
  arrangement.
