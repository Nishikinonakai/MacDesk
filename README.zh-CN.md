...
- **桌面图标开关**——设置里可显示/隐藏 此电脑、用户文件、网络、控制面板、
  回收站；首次启动跟随原生桌面当前的选择。
...
- **网格贴合真实工作区**——读取任务栏的真实大小与位置（四边停靠、小图标、
  隐藏/自动隐藏都认），网格精确用满可用空间，不做保守猜测。
- **首行下沉**（设置 → 桌面）——可选的半行顶部预留，给第三方顶部菜单栏
  让位，首行图标不再被吸顶窗口遮挡。
...
- **开机零闪烁。** 开机自启开着时，MacDesk 会把 Windows 自己的「显示桌面图标」
  状态一并关掉，开机阶段 Explorer 压根不画原生图标——不再"先闪一下原生桌面
  再变成 MacDesk"。自启走登录计划任务，跳过 Windows 对启动项的排队延迟。退出
  MacDesk、关掉自启或卸载都会自动还原原生图标。
...
退出方式：设置 → 高级里的**退出**按钮、**Ctrl+Alt+Q**、或 `--quit`。
